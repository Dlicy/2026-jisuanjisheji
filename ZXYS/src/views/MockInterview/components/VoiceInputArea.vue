<template>
  <div class="input-area" v-if="showInputArea && !allQuestionsCompleted">
    <div class="voice-input-container">
      <!-- 语音识别结果区域 -->
      <div class="voice-result-container" v-if="voiceText || isListening">
      ...
      <div class="voice-tips">
        ...
        <span class="tip-text">点击麦克风按钮，说出您的答案，系统会自动转换为文字</span>
      </div>

      <!-- 浏览器不支持提示 -->
      <div v-if="!isVoiceSupported" class="browser-warning">
        <span class="warning-icon">⚠️</span>
        <span class="warning-text">您的浏览器不支持语音识别，请使用 Chrome 或 Edge 浏览器</span>
      </div>
    </div>
  </div>
</template>
