...
// 更新配置
const APPID = import.meta.env.VITE_API_XFHUMAN_APPID
const APIKEY = import.meta.env.VITE_API_XFHUMAN_APIKEY
const APISECRET = import.meta.env.VITE_API_XFHUMAN_APISECRET
...
const props = defineProps({
  ...
  avatarId: {
    type: String,
    default: '110117005', // 你代码中的默认值
  },
  // TTS 音色，可选
  ttsVcn: {
    type: String,
    default: 'x4_lingxiaoying_assist',
  },
  // 是否自动连接
  autoConnect: {
    type: Boolean,
...
const globalParams = computed(() => ({
  ...
  avatar: {
    avatar_id: props.avatarId, // ← 使用传入的 avatarId
    width: 720,
    height: 1280,
    audio_format: 1,
  },
  tts: {
    vcn: props.ttsVcn, // ← 使用传入的音色
    speed: 50,
    pitch: 50,
    volume: 100,
    emotion: 13,
...
const initAndConnect = async () => {
  ...
  try {
    ...
    avatarPlatform.setGlobalParams(globalParams.value)
    console.log('[数字人] 全局参数已配置 avatar_id:', props.avatarId)
...
const sendTextToAvatar = (text) => {
  if (!isConnected.value) {
    console.warn('[数字人] 未连接，无法发送文本')
    ElMessage.warning('数字人未连接，请稍后再试')
    return false
  }

  if (avatarPlatform && typeof avatarPlatform.writeText === 'function') {
    console.log('[数字人] 发送文本:', text)
    avatarPlatform.writeText(text)
    return true
  } else {
    console.error('[数字人] writeText 方法不可用')
    ElMessage.error('数字人文本发送功能不可用')
    return false
  }
}

// 修改 defineExpose，暴露更多方法给父组件
defineExpose({
  isInitializing,
  sendText: sendTextToAvatar, // 暴露发送文本方法
  interrupt: interruptAvatar, // 暴露打断方法
  isConnected, // 暴露连接状态（可选）
  reconnect: reconnectAvatar, // 暴露重连方法（可选）
})

// 监听 avatarId 变化，自动切换形象
watch(
  () => props.avatarId,
  async (newId, oldId) => {
    if (newId !== oldId && isConnected.value) {
      console.log(`[数字人] 切换形象: ${oldId} -> ${newId}`)
      // 断开当前连接
      if (avatarPlatform) {
        try {
          avatarPlatform.stop()
        } catch (e) {
          console.error('[数字人] 停止连接失败:', e)
        }
      }
      isConnected.value = false
      updateGlobalConnectionState(false)
